...
async function getDashboardStats(req, res) {
    try {
        ...
        const monthlySales = await db_js_1.default.sale.findMany({
            where: {
                shopId,
                createdAt: {
                    gte: startOfMonth,
        ...
        const lowStockProducts = await db_js_1.default.product.findMany({
            where: {
                shopId,
                stock: { lte: 5 }
            },
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const daySales = await db_js_1.default.sale.findMany({
                where: {
                    shopId,
                    createdAt: {
                        gte: start,
        ...
        const groupSales = await db_js_1.default.saleItem.groupBy({
            by: ['productId'],
            _sum: {
                quantity: true,
                total: true
        ...
        for (const gs of groupSales) {
            const prod = await db_js_1.default.product.findFirst({
